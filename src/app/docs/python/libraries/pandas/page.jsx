    ...
    <div className="prose prose-slate dark:prose-invert max-w-none">
      ...
      <section>
        ...
        <p>
          ...
          data structures like DataFrame and Series that make working with structured data fast, 
          easy, and expressive.
        </p>
...
import numpy as np  # Often used together`}</code></pre>
      </section>
...
# Access elements
print(s['a'])      # 10
print(s.iloc[0])   # 10`}</code></pre>

        <h3>DataFrame</h3>
        <p>A DataFrame is a two-dimensional labeled data structure:</p>
        <pre><code>{`# Create DataFrame from dictionary
data = {
...
# Write to JSON
df.to_json('output.json', orient='records', indent=2)`}</code></pre>
...
# Write to SQL
df.to_sql('table_name', conn, if_exists='replace', index=False)`}</code></pre>
      </section>

      <section>
        <h2>Viewing and Inspecting Data</h2>
        <pre><code>{`# First/last rows
...
print(df['column'].value_counts())`}</code></pre>
      </section>
...
# Using dot notation
print(df.Name)  # Works if column name has no spaces`}</code></pre>

        <h3>Selecting Rows</h3>
        <pre><code>{`# By position (iloc)
...
print(df.loc[0:2, 'Name':'Age'])  # Specific rows and columns`}</code></pre>
...
print(df[df['City'].isin(cities)])`}</code></pre>
      </section>
...
df = df.drop(['City', 'Age_Group'], axis=1)`}</code></pre>
...
df = df[df['Age'] > 25]  # Keep only rows where Age > 25`}</code></pre>

        <h3>Modifying Values</h3>
        <pre><code>{`# Modify single value
df.loc[0, 'Age'] = 26
...
df.loc[df['Age'] <= 30, 'Status'] = 'Junior'`}</code></pre>
      </section>
...
print(df.groupby('City')['Age'].apply(age_range))`}</code></pre>
      </section>

      <section>
        <h2>Sorting Data</h2>
        <pre><code>{`# Sort by column
...
# In-place sorting
df.sort_values('Age', inplace=True)`}</code></pre>
      </section>
      ...
      <section>
        ...
        <h3>Concatenation</h3>
        <pre><code>{`# Vertical concatenation (stack rows)
...
result = pd.concat([df1, df2], axis=1)`}</code></pre>
        ...
        <h3>Merge (SQL-style joins)</h3>
        <pre><code>{`df1 = pd.DataFrame({'ID': [1, 2, 3], 'Name': ['Alice', 'Bob', 'Charlie']})
...
# Outer join
merged = pd.merge(df1, df2, on='ID', how='outer')`}</code></pre>
      </section>
...
# Strip whitespace
df['Name'] = df['Name'].str.strip()`}</code></pre>
      </section>
...
df.set_index('Date').resample('M').sum()  # Monthly sum`}</code></pre>
      </section>

      <section>
        <h2>Apply and Map Functions</h2>
        <pre><code>{`# Apply function to column
...
# Replace values
df['City'] = df['City'].replace({'New York': 'NYC', 'Los Angeles': 'LA'})`}</code></pre>
      </section>

      <section>
        <h2>Practical Example: Sales Analysis</h2>
        <pre><code>{`# Create sample sales data
data = {
...
print(summary)`}</code></pre>
      ...
      <section>
        ...
        <ul>
          <li><strong>Use vectorized operations:</strong> Avoid loops; use built-in Pandas methods</li>
          <li><strong>Chain operations:</strong> Use method chaining for cleaner code</li>
          <li><strong>Set appropriate data types:</strong> Use category dtype for categorical data</li>
          <li><strong>Use inplace sparingly:</strong> Prefer assignment for clarity</li>
          <li><strong>Handle missing data explicitly:</strong> Don't ignore NaN values</li>
          <li><strong>Index wisely:</strong> Set meaningful indexes for faster lookups</li>
        </ul>
      </section>
      ...
      <section>
        <h2>Summary</h2>
        <p>
          Pandas is the go-to library for data manipulation in Python. Its powerful DataFrame 
          structure and rich functionality make it essential for data cleaning, transformation, 
          and analysis. Master Pandas to work efficiently with structured data.
        </p>
      </section>
    </div>
  );
